...
async function main() {
  ...
  const initialObserver =
    process.env.RAFFLE_OBSERVER_ADDRESS ||
    process.env.VAULT_OBSERVER_ADDRESS ||
    ethers.ZeroAddress;

  console.log("Deploying RaffleDarkpoolV2Relayer with:", await deployer.getAddress());
  console.log("Initial observer:", initialObserver);
  ...
  const raffle = await factory.deploy(initialObserver);

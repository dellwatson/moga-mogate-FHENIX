...
async function main() {
  ...
  console.log("Signer (must be winner):", signer.address);
  ...
  const raffle = new ethers.Contract(
    ...
    [
      ...
      "function unsafeClaimToVault(string raffleId, address vault, address plaintextOwner) external",
    ],
    signer,
  );
  ...
  } else if (plaintextOwner) {
    tx = await raffle.unsafeClaimToVault(raffleId, vaultAddress, plaintextOwner);
  } else {
    throw new Error("Provide ENC_OWNER_HEX (preferred) or UNSAFE_PLAINTEXT_OWNER");
  }
